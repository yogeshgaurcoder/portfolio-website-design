...
export default function Page() {
  ...
  return (
    <main className="site-shell">
      ...
      <section id="education" className="section container"><div className="section-kicker">06 / EDUCATION</div><div className="education-grid"><div><h2>BUILT ON<br/><span>FOUNDATION.</span></h2><p>Formal education in business administration, computer applications and information technology, backed by hands-on analytics experience.</p></div><div className="edu-list">{education.map((e,i)=><div className="edu-card" key={e[0]}><span>0{i+1}</span><div><h3>{e[0]}</h3><p>{e[1]}</p></div><strong>{e[2]}</strong></div>)}<div className="cert-box"><div className="cert-icon"><Sparkles/></div><div><small>CERTIFICATIONS</small>{certifications.map(c=><p key={c}><CheckCircle2 size={14}/>{c}</p>)}</div></div></div></div></section>
